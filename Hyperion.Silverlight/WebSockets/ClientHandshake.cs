...
using Hyperion.Silverlight.Text;
...
namespace Hyperion.Silverlight.WebSockets
{
    ...
    public class ClientHandshake
    {
        ...
        private static byte[] GenerateKey3(FastRandom random)
        {
            ...
            {
                key += Convert.ToString(random.Next(21, 126));
            }
        ...
        public bool IsValid(string host, string origin)
        {
            return !string.IsNullOrEmpty(ResourceName) &&
                   string.Compare(Upgrade, DefaultUpgrade, StringComparison.OrdinalIgnoreCase) == 0 &&
                   string.Compare(Connection, DefaultConnection, StringComparison.OrdinalIgnoreCase) == 0 &&
        ...
        private void SetProperty(string lineInHandshake)
        {
            ...
            {
                var sepereatorIndex = lineInHandshake.IndexOf(Seperator);
                if (sepereatorIndex > -1)
                {
                    var valueStartIndex = sepereatorIndex + 2;
                    var fieldName = lineInHandshake.Substring(0, sepereatorIndex);
